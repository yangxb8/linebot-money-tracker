...
- Q: Web functional verification surface for v1? → A: API/route checks for core mutations + minimal browser smoke (auth gate + one signed-in page)
- Q: Web v1 journey scope? → A: Auth smoke + expense overview + wish list + bot-behavior settings + budgets + categories (periodic expenses later)
- Q: How strongly to enforce test-suite expansion on future features? → A: Soft policy for expansion + hard CI fail if any suite fails (no mandatory test-diff gate)
- Q: Bot receipt/image journeys in v1 functional suite? → A: Text expense + reply-edit + wish accept/decline only; image later
- Q: Where do web PR-lane tests get auth/data? → A: PR lane fully mocked (auth + data); remote test tenant only for optional deep lane
...
5. **Given** v1 scope, **When** the bot functional suite runs, **Then** receipt/image journeys are not required to pass as part of that suite.

---

### User Story 2 - Catch web dashboard regressions before merge (Priority: P1)

As a maintainer, when someone changes the web dashboard, automated checks verify that signed-in users can still complete core money-management tasks (view expenses, manage wish list, budgets, categories, and bot-behavior settings), and that signed-out users cannot access protected areas. Periodic expenses are out of scope for the first delivery of this suite.
...
**Independent Test**: Run the automated web functional suite with mocked auth and data stores (no live remote ledger); confirm protected-route denial via browser smoke, one signed-in page load, and API/route happy paths for expenses, wish list, settings, budgets, and categories.

**Acceptance Scenarios**:

1. **Given** a visitor with no session, **When** browser smoke opens a protected dashboard URL, **Then** they are sent to sign-in and no private data is shown.
2. **Given** a signed-in test user, **When** browser smoke opens one authenticated page (expense overview or equivalent), **Then** the page loads successfully without an auth bounce.
3. **Given** a signed-in test user, **When** they add or update a wish-list item through the documented product API/route surface, **Then** a subsequent read shows the change.
4. **Given** a signed-in test user, **When** they save bot-behavior settings through the documented product API/route surface, **Then** a subsequent read shows the saved values.
5. **Given** a signed-in test user, **When** expense overview data is requested through the documented product API/route surface, **Then** the response succeeds and returns ledger content or an empty state.
6. **Given** a signed-in test user, **When** they read or update budget data through the documented product API/route surface, **Then** the response succeeds and reflects the expected budget state.
7. **Given** a signed-in test user, **When** they create or update a category through the documented product API/route surface, **Then** a subsequent read shows the change.

---

### User Story 3 - Continuous checks on every change review (Priority: P2)

As a maintainer, every proposed change automatically runs the fast functional suites (bot + web) alongside existing unit checks, using fully mocked auth/data and external services—not production secrets or a live remote test tenant—so reviews get a clear pass/fail signal.
...
**Independent Test**: Open a sample change that does not alter product behavior; confirm the pipeline runs bot unit tests, web unit tests, and the new functional suites, all green without real LINE/AI/Supabase production credentials or a remote test tenant.
...
3. **Given** a pull request for a user-facing change with no corresponding test updates, **When** reviewed against project policy, **Then** it is treated as incomplete relative to the constitution and agent rules (soft enforcement via instructions and review—not an automated “must touch test files” CI gate).
4. **Given** any pull request where an existing unit or functional suite fails, **When** continuous checks run, **Then** the pipeline fails (hard gate).
...
- How does the system handle flaky browser or network timing in UI checks? Suites MUST prefer stable assertions (visible outcomes, persisted data) and keep browser smoke to auth gate + one signed-in page.
- What about receipt/image bot flows? Out of scope for v1 bot functional suite; covered by existing unit tests until a later expansion adds a mocked image scenario.
...
- How are destructive tests isolated? The PR (fast) lane MUST use fully mocked auth and data stores. A remote dedicated test tenant is reserved for the optional deep lane only—never the live household production data used by real users.
...
- **FR-017**: Receipt/image expense journeys are out of scope for the first delivery of the bot functional suite (may be added later with mocked vision/AI).
...
- **FR-005**: The web functional suite MUST cover at least: expense overview load, wish-list create/update visibility, bot-behavior settings save/reload, budgets read/update, and categories create/update via the documented product API/route surface.
- **FR-006**: Web PR-lane functional checks that require a signed-in actor MUST simulate auth via a mocked test identity/session—not interactive personal LINE login and not a live remote database session.
- **FR-018**: The PR (fast) verification lane MUST fully mock auth and data stores for web functional checks. A remote dedicated test tenant MAY be used only in the optional deep/scheduled lane (FR-012), not as a PR merge requirement.
- **FR-014**: Web v1 functional coverage MUST use API/route checks for core mutations and reads (wish list, settings, expense overview, budgets, categories), plus minimal browser smoke limited to the unauthenticated auth gate and one signed-in page load. Full dual API+browser coverage of every journey is out of scope for v1.
- **FR-015**: Periodic-expense journeys are out of scope for the first delivery of the web functional suite (may be added in a later expansion).
...
- **FR-009**: Repository agent instructions MUST state that every new user-facing feature is incomplete until the automated test suite is expanded to cover that feature’s primary acceptance scenarios (soft policy for humans/agents and PR review).
...
- **FR-016**: Continuous checks MUST hard-fail when any required unit or functional suite fails. Continuous checks MUST NOT require that every user-facing PR diff also touches test files (no mandatory test-diff gate).
...
- **Test identity**: A simulated signed-in user/tenant used by automated web checks in the PR lane (mocked); a real isolated tenant may appear only in the optional deep lane.
- **Verification lane**: A grouping of checks—fast PR lane (fully mocked auth/data and externals) vs optional deep lane (scheduled/real services and/or remote test tenant).
...
- **SC-002**: Maintainers can verify protected-route denial and one signed-in page load via browser smoke, plus expense overview, wish-list update, settings save/reload, budgets, and categories via API/route checks, all in under 10 minutes without interactive personal login.
...
- **SC-004**: After this feature ships, agent instructions and planning artifacts require explicit automated test-expansion items for user-facing work; enforcement of “did this PR add tests?” remains soft (review/policy), while failing suites remain a hard CI failure.
...
- Web authentication in the PR lane uses a mocked test identity/session rather than automating LINE Login or calling a live auth provider. A remote test tenant is reserved for the optional deep lane only.
- Browser automation in v1 is limited to auth-gate redirect and one signed-in page load; core web journeys are proven through API/route checks. Deeper UI edge cases stay in unit tests.
- Nightly real-AI and cross-surface packs are optional (P3) and may ship after the fast suites. Mocked receipt/image bot functional scenarios are also deferred past v1.
- “Expand the test suite” for a feature means adding or updating automated tests for that feature’s primary acceptance scenarios; pure refactors with no behavior change may only require existing suites to stay green. Whether a PR added tests is soft-enforced; failing suites are hard-enforced in CI.
